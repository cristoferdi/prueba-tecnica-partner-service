...
- *Opción A (Docker):* Ejecutar `docker compose up -d` en la raíz para levantar PostgreSQL en `localhost:5432`. 
...
- Ir a `backend/` → `cp .env.example .env` (ajustar credenciales si no se usa Docker) → `./mvnw spring-boot:run`. 
- La API responderá en `http://localhost:8080/api/v1` y Swagger UI en `http://localhost:8080/swagger-ui/index.html`.

**3. Frontend (React/Vite).** 
- Ir a `frontend/` → `cp .env.example .env` → `npm install` → `npm run dev`. 
